...
export class Formacion {
   ...
   */
  posicionDeCasilla(columna, fila) {
    ...
    return {
      x: this.anclaX + centrado * FORMACION.SEPARACION_X * respiro,
      y: this.anclaY + fila * FORMACION.SEPARACION_Y,
    };
  }

  /**
   * Elige un enemigo que pueda disparar: solo los que no tienen a ningún
   * compañero vivo por debajo. Disparar desde la fila de atrás sería
   * dispararse a sí mismos.
   */
  elegirTirador() {
    const candidatos = [];
    for (const e of this.vivos) {
      if (e.recarga > 0) continue;
      const tapado = this.vivos.some(
        (otro) => otro.columna === e.columna && otro.fila > e.fila
      );
      if (!tapado) candidatos.push(e);
    }
